...
IFX_INTERRUPT(cc61_pit_ch0_isr, CCU6_1_CH0_INT_VECTAB_NUM, CCU6_1_CH0_ISR_PRIORITY)
{
    ...
    for (uint8_t i = 0; i < SEEKFREE_ASSISTANT_SET_PARAMETR_COUNT; i++)
    {
        ...
        if (seekfree_assistant_parameter_update_flag[i])
        {
            ...
            {
            case 0:
                line_kp = seekfree_assistant_parameter[0];
                break;
            case 1:
                line_ki = seekfree_assistant_parameter[1];
                break;
            case 2:
                line_kd = seekfree_assistant_parameter[2];
                break;
            case 3:
                set_line_speed = seekfree_assistant_parameter[3];
                break;
            case 4:
                delta_y = seekfree_assistant_parameter[4];
                break;
            case 5:
                angle_yaw = seekfree_assistant_parameter[5];
                break;
            }
            //--------------通过DEBBUG串口发送信息
